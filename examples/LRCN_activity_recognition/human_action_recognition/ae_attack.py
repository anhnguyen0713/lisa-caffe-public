...
AE_PATH = "~/action_recog/UCF101/lrcn_fgsm_8_rate_1_crf_30"
NET_PROTOTXT = "~/action_recog/lisa-caffe-public/examples/LRCN_activity_recognition/deploy_lstm_ae.prototxt"
WEIGHTS_PATH = "~/action_recog/lisa-caffe-public/examples/LRCN_activity_recognition/RGB_lstm_model_iter_30000.caffemodel"
OUTPUT = "results/rgb_center_fgsm_8_rate_3.txt"
...
for i_line, line in enumerate(f_lines):
	...
	for i in range(0, len(frames), CLIP_LENGTH):
		...
		clip_data = np.array(list(clip_data[i] for i in [4,14,24,34,44,54,64,74]))
		...
		# Set input blobs by read image data, label and markers
		net.blobs['data'].data[...] = data_in
		net.blobs['label'].data[...] = label_in
		net.blobs['clip_markers'].data[...] = clip_markers_in

		# Forward input data from data layer to loss layer
		res = net.forward()
